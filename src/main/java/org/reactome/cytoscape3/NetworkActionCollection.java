...
class NetworkActionCollection implements NetworkAboutToBeDestroyedListener
{
    ...
    public static void setSelectedOrUnselected(CyNetwork network, CyNode node, boolean value)
    {
        Long nodeSUID = node.getSUID();
        CyTable nodeTable = network.getDefaultNodeTable();
        nodeTable.getRow(nodeSUID).set("selected", value);
    }
    private static void setSelectedOrUnselected(CyNetworkView view, View<CyNode> nodeView, boolean value)
    {
        setSelectedOrUnselected(view.getModel(), nodeView.getModel(), value);
    }
    
    public static void selectNodesFromList(CyNetwork network, Collection<CyNode> nodes, boolean value)
    {
        ...
        public CyMenuItem createMenuItem(final CyNetworkView view)
        {
            ...
            clusterMenuItem.addActionListener(new ActionListener()
            {
                ...
                public void actionPerformed(ActionEvent e)
                {
                    ...
                    {
                        if (clustering != null
                                && !clustering.equals(TableFormatterImpl
                                        .getSpectralPartitionCluster()))
                        {
                            CySwingApplication desktopApp = PlugInScopeObjectManager
                                    .getManager().getCySwingApp();
            ...
            fetchFIAnnotationsMenu.addActionListener(new ActionListener()
            {
                ...
                public void actionPerformed(ActionEvent e)
                {
                    ...
                    try
                    {
                        ...
                        ServiceReference servRef = context
                                .getServiceReference(FIVisualStyle.class
                                        .getName());
    ...
    private void doModuleSurvivalAnalysis(CyNetworkView view)
    {
        ...
        if (nodeToModule == null || nodeToModule.isEmpty())
            return; //There is not data for analysis.
        ...
        try
        {
            ...
            {
                Map<String, Object> nodeToSamples = tableHelper.getNodeTableValuesByName(view.getModel(), "samples", String.class);
                ...
                survivalHelper.doSurvivalAnalysis((Map<String, Integer>)nodeToModule, nodeToSampleSet);
            }
            else if (dataType.equals(TableFormatterImpl.getMCLArrayClustering()))
            {
                ...
                if (moduleToSampleToValue == null || moduleToSampleToValue.size() == 0) {
                    PlugInUtilities.showErrorMessage("No sample information has been provided. Survival analysis cannot be done.", 
    ...
    public void clusterFINetwork(CyNetworkView view)
    {
        try
        {
            ...
            NetworkClusterResult clusterResult = service
                    .cluster(edgeList, view);
            ...
            try
            {
                showModuleInTab(nodeToCluster, nodeToSamples, clusterResult
                        .getModularity(), view);
    ...
    public void handleEvent(NetworkAboutToBeDestroyedEvent e)
    {
        ...
        boolean isReactomeFINetwork = netTable.getRow(e.getNetwork().getSUID())
    ...
    protected void annotateNetwork(final CyNetworkView view, final String type)
    {
        ...
        if (!linkers.isEmpty())
        {
            ...
            int reply = JOptionPane.showConfirmDialog(desktopApp.getJFrame(), "Linkers have been used in network construction.\n" +
                                        "Including linkers will bias results. Would you like to include them anyway?", "Include Linker Genes", JOptionPane.YES_NO_CANCEL_OPTION);
        ...
        Thread t = new Thread()
        {
            public void run()
            {
                ...
                PlugInScopeObjectManager.getManager().getCytoscapeDesktop().setGlassPane(progPane);
                PlugInScopeObjectManager.getManager().getCytoscapeDesktop().getGlassPane().setVisible(true);
                try
                {
                ...
                PlugInScopeObjectManager.getManager().getCytoscapeDesktop().getGlassPane().setVisible(false);
            }
        };
        t.start();
    }
    ...
    private void displayModuleAnnotations(List<ModuleGeneSetAnnotation> annotations,
            ...
            boolean isForModule)
    {
        ResultDisplayHelper.getHelper().displayModuleAnnotations(annotations, view, type, isForModule);
    }
    private Map<String, Integer> extractNodeToModule(CyNetworkView view)
    {
        ...
        for (CyNode node : view.getModel().getNodeList())
        {
            ...
            //Since nodes which are unlinked will have null value for module (as may some other nodes),
        ...
        if (!linkers.isEmpty())
        {
            ...
            if (reply == JOptionPane.NO_OPTION)
            {
                ...
                {
                    JOptionPane.showMessageDialog(desktopApp.getJFrame(), "No genes remain after removing linkers. Annotation cannot be performed.", "Cannot Annotate Modules", JOptionPane.INFORMATION_MESSAGE);
                    return null;
                }
            }
        }
    ...
    private Integer applyModuleSizeFiler(Map<String, Integer> nodeToModule)
    {
        ...
        Integer input = (Integer) JOptionPane.showInputDialog(PlugInScopeObjectManager.getManager().getCytoscapeDesktop(), 
